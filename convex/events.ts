...
export const joinWaitingList = mutation({
  ...
  handler: async (ctx, { eventId, userId }) => {
    // Rate limit check
    // const status = await components.rateLimiter.limit(ctx, "queueJoin", { key: userId });
    // if (!status.ok) {
    ...
    const { available } = await ctx.runQuery(api.events.checkAvailability, { eventId }) as { available: boolean };
  ...
  handler: async (ctx, { userId }) => {
    ...
    const ticketsWithEvents = await Promise.all(
      tickets.map(async (ticket) => {
        ...
        return {
          ...ticket,
          event: event || null, // Handle case where event might be deleted
        };
      })
    );
