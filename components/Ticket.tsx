...
export default function Ticket({ ticketId }: { ticketId: Id<"tickets"> }) {
  ...
  const organizerProfile = useQuery(
    ...
    ticket?.event?.userId ? { userId: ticket.event.userId } : "skip"
  );
  const imageUrl = useStorageUrl(ticket?.event?.imageStorageId);
  const organizerLogoUrl = useStorageUrl(organizerProfile?.organizerLogoStorageId);
      ...
      >
        ...
        <div className="text-right">
          <p className="text-xs text-gray-500">
            Kjøpt: {new Date(ticket.purchasedAt).toLocaleDateString("nb-NO")}
          </p>
          <span
            className={`text-sm font-medium ${ticket.event.is_cancelled ? "text-red-600" : "text-blue-600"}`}
          >
            {ticket.event.is_cancelled ? "Kansellert" : "Gyldig billett"}
          </span>
        </div>
      </div>
    </div>
